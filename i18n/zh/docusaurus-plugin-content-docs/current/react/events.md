...
:::note
React 封装器使用 `onCamelCase` 形式的 prop 名称（例如 `onAfterAction`），而 JS Spreadsheet API 在事件总线上使用 `camelCase` 形式的事件名称（例如 `afterAction`）。命令式 API 请参见 [JS API Events 参考](api/overview/events_overview.md)。
:::
