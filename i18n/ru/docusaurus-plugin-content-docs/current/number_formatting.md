...
- `dateFormat` — (необязательный) формат отображения дат в виде строки, по умолчанию `"%d/%m/%Y"`. Подробнее на странице АПИ [`localization`](api/spreadsheet_localization_config.md)
