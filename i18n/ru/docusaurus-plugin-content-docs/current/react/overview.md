...
`ReactSpreadsheet` — это декларативная React-обёртка для виджета DHTMLX Spreadsheet. Предоставляет компонентный АПИ, где пропсы описывают состояние таблицы, а обёртка обрабатывает синхронизацию с базовым виджетом.
...
| **Состояние** | `search`, `theme`, `loadUrl` | Применяются через специализированные АПИ виджета без побочных эффектов. |
