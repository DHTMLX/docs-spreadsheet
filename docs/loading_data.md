...
By default, Spreadsheet expects data in JSON format. To load data from an external source, use the [load()](../api/spreadsheet_load_method) method. It takes the URL of the file with data as a parameter:
...
You can also load data in the CSV format. For this, you need to call the [load()](../api/spreadsheet_load_method) method and pass the name of the format ("csv") as the second parameter:
...
- specify the [importModulePath](../api/spreadsheet_importmodulepath_config) option in the Spreadsheet configuration and set the path to the **worker.js** file in one of the two ways:
...
To load data from an Excel file, pass a string with the type of the extension ("xlsx") as the second parameter of the [load()](../api/spreadsheet_load_method) method :
...
To load data from a local source, make use of the [parse()](../api/spreadsheet_parse_method) method. Pass a [predefined data set](#preparing-data) as a parameter of this method:
...
To save the current state of a spreadsheet, use the [serialize()](../api/spreadsheet_serialize_method) method. It converts data into an array of JSON objects. Each JSON object contains the configuration of a cell.
...
- specify the [exportModulePath](../api/spreadsheet_exportmodulepath_config) option in the Spreadsheet configuration and set the path to the **worker.js** file in one of the two ways:
...
Once you've adjusted the required sources, you can use the related [xlsx()](../api/export_xlsx_method) API method of the Export object to export data of the component, as in:
...
By default, the name for an exported file is "data". You can provide your own name for an exported file. For this, you need to pass a custom name as a parameter of the [xlsx()](../api/export_xlsx_method) method, as in:
